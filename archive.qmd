...
::: callout-tip
The complete 2023 version of this workshop is available on Zenodo. [![DOI](https://zenodo.org/badge/601148138.svg)](https://zenodo.org/badge/latestdoi/601148138)<br>
The corresponding recording of this workshop is available [here](https://www.dropbox.com/s/5lqwsn63v93f5dn/rpackage-00.00.17.848-00.56.25.896.mp4?dl=0). Disclaimer: In the recording, there's an error, caused by removing the `GitHub` repo `roundR` that was created in the exercise materials and re-creating it with the exact same `roundR` name during the lecture. That is why `Zenodo` does not accept it a second time for a DOI. This makes sense, as you would not want to create a new DOI for content with the same name at the same location, nor use the same DOI for content with different time codes and temporal origin. For your repository, however, this would not be an issue. 
:::
---
...
# Motivating example
In order to demonstrate the creation of an R-package, we will identify some odd behavior by `R`:
```{r}
#| warning: false
...
```
We can see that `R` rounds - by default -  the number `1.5` to integer `2` and the number `2.5` also to integer `2`. The reason for this behavior is the IEC 60559 standard where a 5 is expected to be rounded to the even digit. 
...
# rounder <- function(x){
#   diff <- x - floor(x)
#   ifelse(dplyr::between(diff, 0, 0.5), floor(x), ceiling(x))
# }
```
